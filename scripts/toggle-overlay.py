...
with (ROOT/'logs/overlay.lock').open('w') as lock:
 ...
 with (ROOT/'logs/overlay.log').open('a') as out:
  subprocess.Popen(['chromium','--user-data-dir='+str(ROOT/'logs/overlay-profile'),'--class=jarvis-overlay','--no-first-run','--disable-extensions','--disable-background-networking','--no-default-browser-check','--disable-background-mode','--app=http://127.0.0.1:7421/jarvis-overlay','--window-size=640,460'],stdout=out,stderr=out,start_new_session=True)
 for _ in range(75):
