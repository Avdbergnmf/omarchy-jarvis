...
## Checklist
- [ ] Inventory current slash commands (`/report`, `/feature`, `/backlog`, `/dispatch`, …) into a single registry the UI can read (JSON endpoint or static list kept in sync — document)
- [ ] Overlay: dropdown/palette under the textbox; filter by prefix; insert completion
- [ ] Keep existing command behavior unchanged when submitted
- [ ] Tests for filter + keyboard; README one-liner
- [ ] PROGRESS + short ADR if non-obvious; QUEUE → done; move to `docs/assignments/done/`
