...
- **Status:** queued
- **Area:** area:actions
- **parallel-ok:** YES (ADR-047 default — preference records in `actions/core.py` + `~/.config/jarvis/`; `Gate: control-plane` here groups the queue, it does not change the authority surface)
